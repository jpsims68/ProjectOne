...
Read the arrow as "is allowed to use". One way only: a slice never imports another slice
directly, and platform never imports a slice. Cross-slice dependency goes through a
versioned contract, so one slice can change internally without breaking others.
...
`release/` `evidence/` `continuity/`

Documentation never creates or changes a decision. Where documentation and its governing
source diverge, the source wins and the divergence is a defect.
