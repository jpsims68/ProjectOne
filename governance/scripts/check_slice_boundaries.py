...
for name in slice_names:
    ...
    if not mf.exists():
        viol.append(f"[VSA-1] slices/{name}: missing slice.manifest.json — a slice without a manifest is ungoverned code")
        continue
    try:
        m = json.loads(mf.read_text()); manifests[name] = m
    except Exception as e:
        viol.append(f"[VSA-1] slices/{name}/slice.manifest.json is not valid JSON: {e}"); continue
    for field in ("sliceId", "ownerRole", "consumedContracts", "producedContracts", "lifecycleState", "riskClass"):
        if field not in m:
...
for name in slice_names:
    for f in sources(SLICES/name):
        ...
        for other in slice_names:
            if other == name:
                continue
            if re.search(rf"(from|import|require\()\s*['\"]?[\w./]*slices[./]{re.escape(other)}\b", text):
                viol.append(f"[VSA-3] {f.relative_to(ROOT)} imports slice '{other}' directly — cross-slice dependency must go through /contracts")

# Rule 2 — platform must not depend on a slice
for f in sources(PLATFORM):
    text = f.read_text(errors="ignore")
    for name in slice_names:
        if re.search(rf"(from|import|require\()\s*['\"]?[\w./]*slices[./]{re.escape(name)}\b", text):
            viol.append(f"[VSA-3] {f.relative_to(ROOT)} imports slice '{name}' — platform may never depend on a slice (one-way direction)")
...
for name, m in manifests.items():
    for c in m.get("consumedContracts", []):
        ...
        if cid not in produced and CONTRACTS.exists() and not any(CONTRACTS.rglob(f"{cid}*")):
            viol.append(f"[VSA-3] slices/{name}: consumes contract '{cid}' which is not produced by any slice and has no definition in /contracts")
...
def dfs(n, path):
    ...
    for m in graph[n]:
        if color[m] == GREY:
            viol.append(f"[VSA-3] contract dependency CYCLE: {' -> '.join(path + [n, m])} — dependencies must be acyclic")
        elif color[m] == WHITE:
            dfs(m, path + [n])
    color[n] = BLACK
for n in list(graph):
...
print("=" * 72)
print(f"slices found: {len(slice_names)}  {slice_names if slice_names else '(none yet — scaffold stage)'}")
if viol:
